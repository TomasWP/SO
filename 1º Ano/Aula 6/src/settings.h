...
/**
 *  \brief dummy patient ID (can be used to terminate simulation)
 */
#define DUMMY_ID MAX_PATIENTS
 ...
 */
#define check_valid_patient_id(id) do { if(id > MAX_ID && id != DUMMY_ID) { printf("\e[31;01m[%s, line %d, function %s] ERROR: invalid patient ID %u!\e[0m\n", __FILE__, __LINE__, __FUNCTION__, id); exit(EXIT_FAILURE); } } while(0)

/**
 *  \brief nurse id verification test
 */
#define check_valid_nurse_id(id) do { if(id >= MAX_NURSES) { printf("\e[31;01m[%s, line %d, function %s] ERROR: invalid nurse ID %u!\e[0m\n", __FILE__, __LINE__, __FUNCTION__, id); exit(EXIT_FAILURE); } } while(0)
