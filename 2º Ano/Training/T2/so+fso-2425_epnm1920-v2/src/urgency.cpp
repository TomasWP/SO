...
void term_simulation(int np) {
   ...
   //cond_destroy(&hd->all_patients->patiend_done);
...
int doctor_iteration(int id) // return value can be used to request termination
{
   ...
   cond_broadcast(&hd->all_patients->patiend_done);
...
void patient_goto_urgency(int id)
{
   ...
   cond_init(&hd->all_patients->patiend_done, NULL);
...
void patient_wait_end_of_consultation(int id)
{
   ...
   while(hd->all_patients[id].done == 0){
      cond_wait(&hd->all_patients->patiend_done, &hd->all_patients->patient_mutex);
   }
   mutex_unlock(&hd->all_patients->patient_mutex);
}
...
void patient_life(int id)
{
   ...
   cond_destroy(&hd->all_patients[id].patiend_done);
...
int main(int argc, char *argv[])
{
   ...
   for(int i = 0; i < nnurses; i++){
      nurses_id[i] = i;
      thread_create(&all_nurses[i], NULL, nurse_thread, (void*)&nurses_id[i]);
   }
   ...
   for(int i = 0; i < ndoctors; i++){
      doctors_id[i] = i;
      thread_create(&all_doctors[i], NULL, doctors_thread, (void*)&doctors_id[i]);
   }
