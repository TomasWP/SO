...
int doctor_iteration(int id) // return value can be used to request termination
{
   ...
   cond_broadcast(&hd->all_patients[patient].isdone,&hd->all_patients[patient].patient_mutex);
...
void patient_goto_urgency(int id)
{
   ...
   cond_init(&hd->all_patients[id].isdone,NULL);
...
void patient_wait_end_of_consultation(int id)
{
   ...
   while(&hd->all_patients[id].isdone == 0){
      cond_wait(&hd->all_patients[id].isdone, &hd->all_patients[id].patient_mutex);
   }
   mutex_lock(&hd->all_patients[id].patient_mutex);
}
...
void patient_life(int id)
{
   ...
   cond_destroy(&hd->all_patients[id].isdone);
...
void* nurse_thread (void* arg){
   ...
   int return_code = 0;
   while(return_code == 0){
      return_code = nurse_iteration(id);
   }
   return NULL;
}
...
int main(int argc, char *argv[])
{
   ...
   int patientids[npatients];
   for(int i=0; i<npatients; i++){
      patientids[i]=i;
      thread_create(&allpatients[i],NULL,patient_thread;(void*)patientids[i]);
   }


   pthread_t allnurses[nnurses];
   int nurseids[nnurses];
   for(int i=0; i<nnurses; i++){
      nurseids[i]=i;
      thread_create(&allnurses[i],NULL,nurse_thread,(void*)nurseids[i]);
   }

   pthread_t alldoctors[ndoctors];
   int doctorids[ndoctors];
   for(int i=0; i<ndoctors;i++){
      doctorids[i]=i;
      thread_create(&alldoctors,NULL,doctor_thread,(void*)doctorids[i]);
   }
