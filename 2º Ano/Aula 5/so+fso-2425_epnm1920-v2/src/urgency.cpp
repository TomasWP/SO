...
typedef struct
{
   ...
   pthread_cond_t patient_is_done;
} Patient;
...
int doctor_iteration(int id) // return value can be used to request termination
{
   ...
   cond_broadcast(&hd->all_patients[patient].patient_is_done);
...
void patient_wait_end_of_consultation(int id)
{
   ...
   while(hd->all_patients[id].done == 0){
         cond_wait(&hd->all_patients[id].patient_is_done, &hd->all_patients[id].patient_mutex);
   }
...
int main(int argc, char *argv[])
{
   ...
   for(int i = 0; i < nnurses; i++){
      ...
      nurses_ids[i] = i;
      thread_create(&all_nurses[i], NULL, nurse_thread, (void*)&nurses_ids[i]);
   }
   ...
   for(int i = 0; i < ndoctors; i++){
      ...
      doctors_ids[i] = i;
      thread_create(&all_doctors[i], NULL, doctor_thread, (void*)&doctors_ids[i]);
   }
   ...
   for(int i = 0; i < nnurses; i++){
      ...
      patients_ids[i] = i;
      thread_create(&all_patients[i], NULL, patient_thread, (void*)&patients_ids[i]);
   }
