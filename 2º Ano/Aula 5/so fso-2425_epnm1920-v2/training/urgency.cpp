...
void patient_goto_urgency(int id)
{
   ...
   mutex_init(&hd->all_patients->patient_acess, NULL);
...
void patient_wait_end_of_consultation(int id)
{
   ...
   pthread_mutex_lock(&hd->all_patients[id].patient_acess);
   while (hd->all_patients[id].done == 0) {
      pthread_cond_wait(&hd->all_patients[id].patient_done, &hd->all_patients[id].patient_acess);
   }
   pthread_mutex_unlock(&hd->all_patients[id].patient_acess);
...
void* nurse_thread(void* arg){
   ...
   int id = *(int*)arg;
   int return_code = 0;
   while(!is_closed_pfifo(&hd->triage_queue) && return_code == 0){
      nurse_iteration(id);
   }
   return NULL;
}
...
void* doctor_thread(void* arg){
   ...
   int id = *(int*)arg;
   int return_code = 0;
   while(!is_closed_pfifo(&hd->doctor_queue) && return_code == 0){
      doctor_iteration(id);
   }
   return NULL;
}
...
int main(int argc, char *argv[])
{
   ...
   for(int i=0; i < npatients; i++){
      thread_create(&all_patients[i], NULL, patient_thread, (void*) &i);
   }

   pthread_t all_nurses[nnurses];
   for(int i=0; i < nnurses; i++){
      thread_create(&all_nurses[i], NULL, nurse_thread, (void*) &i);
   }

   pthread_t all_doctors[ndoctors];
   for(int i=0; i < ndoctors; i++){
      thread_create(&all_doctors[i], NULL, doctor_thread, (void*) &i);
   }
